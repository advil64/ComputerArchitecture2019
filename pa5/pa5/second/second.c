...
struct variable *specialFindVar(char *);
void freeMyMatrix(int **, int);
...
void createGraycode(){
  ...
  int *myRow;
	...
	for(x = 0; x < rows; x++){
    //create a new row
    myRow = malloc((numOfOutputVars+numOfVars)*sizeof(int));
		//find the graycode
		gray = x ^ (x >> 1);
		...
		//if the gate traversal output is null, we gucci, otherwise rearrange
		tempGate = gateTraversal();
		//find the necessary outputs
		while(tempGate != NULL){
			//if the gate is the head, push all the way to the back
			if(tempGate == Head){
				//reassign head
				Head = tempGate -> next;
				tempGate -> next = NULL;
				...
				//our gate is in the middle
				while(prevTempGate -> next != tempGate){
					prevTempGate = prevTempGate -> next;
				}
				//skip over temp gate
				prevTempGate -> next = tempGate -> next;
				tempGate -> next = NULL;
				//assign tempGate to Tail's next
				Tail -> next = tempGate;
				Tail = Tail -> next;
			}
			//call the method again to see if the problem is resolved
			tempGate = gateTraversal();
		}
...
//returns the gate that needs to be pushed to the end, NULL if there is no problem
struct Gate *gateTraversal(){
	//store the operation
	...
	while(temp != NULL){
		...
		if(strcmp(operation, "NOT") == 0){
			...
			if(!(isdigit(*name1))){
				var1 = specialFindVar(name1);
				//we have successfully found the correct variable
				if(var1 != NULL){
					value1 = var1 -> value;
				} else{
					//this gate needs to be pushed to the end of its list
					return temp;
				}
			} else{
...
struct variable *specialFindVar(char *toFind){
	//counters
	int i = 0;
